...
public final class PersonUtil
{
    ...
    /** The ratio of the female given names, the rest are male. */
    private static final double FEMALE_RATIO = 0.4;

    /** Nextname. */
    public static String nextName(RandomGenerator rng)
    {
        StringBuilder sb = new StringBuilder();
        sb.append(CommonUtil.randChoose(rng, LAST_NAMES));
        if (rng.nextDouble() < FEMALE_RATIO) {
            sb.append(CommonUtil.randChoose(rng, FIRST_NAMES_FEMALE));
        }
        else {
            sb.append(CommonUtil.randChoose(rng, FIRST_NAMES_MALE));
        }
        return sb.toString();
    }
}
